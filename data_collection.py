...
def getting_detailed_statistics(
    ...
    :param game_status: str - статус игры
    :param statistic:
    :return: dict
    """
    ...
    clean_sheets: int = 1 if game_status == 'win' and missed_balls == 0 and minute >= 60 else 0
...
def fill_data_player(
    ...
    for column in range(0, 7):
        ...
        if re.search('\([A-Z]{3}\)', info_by_table.iat[row, column]):
            club: str = re.search('\([A-Z]{3}\)', info_by_table.iat[row, column]).group()[1:-1]
        else:
            squad_list.append({})
            continue
        split_name_player = info_by_table.iat[row, column].split(":")
        split_name_player = split_name_player[1] if len(split_name_player) > 1 else split_name_player[0]
        name = re.split('\(', split_name_player.split()[0])[0]
